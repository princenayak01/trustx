...
export default function TrustXLiveCenter() {
  ...
  async function loadReviews() {
    ...
    try { const response = await fetch('/api/reviews'); const json = await response.json(); if (!response.ok) throw new Error(json?.error?.message || 'Unable to load reviews'); setReviews(json.data || []) }
  ...
  async function loadAnalytics() {
    setBusy(true); setMessage('')
    try { const response = await fetch('/api/analytics'); const json = await response.json(); if (!response.ok) throw new Error(json?.error?.message || 'Unable to load analytics'); setAnalytics(json.data?.summary || json.data || null) }
  ...
  async function createReview(decision: 'APPROVE' | 'REJECT' | 'ESCALATE' | 'REQUEST_REVERIFICATION') {
    setBusy(true); setMessage('')
    try {
      const response = await fetch('/api/reviews', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ screeningId: DEMO_SCREENING_ID, decision, notes: 'Decision recorded from TrustX review center.' }) })
      const json = await response.json(); if (!response.ok) throw new Error(json?.error?.message || 'Unable to record review')
      setMessage(`Review decision ${decision.replaceAll('_', ' ').toLowerCase()} recorded.`); await loadReviews()
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to record review') }
    finally { setBusy(false) }
  }

  function exportReport() { window.open(`/api/reports/${DEMO_SCREENING_ID}`, '_blank', 'noopener,noreferrer') }
  function printReport() { window.open(`/api/reports/${DEMO_SCREENING_ID}`, '_blank', 'noopener,noreferrer'); setMessage('Report data opened. Use your browser Print → Save as PDF.') }

  return <>
    <button className="live-center-trigger" onClick={() => { setOpen(true); loadReviews() }} aria-label="Open live TrustX center"><ShieldAlert size={15} /> Live center</button>
    {open && <div className="live-center-backdrop" onClick={() => setOpen(false)}>
      <section className="live-center" onClick={(event) => event.stopPropagation()}>
        ...
        {tab === 'report' && <div className="live-center-content report-actions"><div className="report-card"><FileText size={22} /><div><strong>Screening report</strong><span>Live report payload from /api/reports/{DEMO_SCREENING_ID}</span></div></div><button className="button button-primary" onClick={exportReport}><Download size={15} /> Open report JSON</button><button className="button button-secondary" onClick={printReport}><Download size={15} /> Print / Save as PDF</button></div>}
      </section>
    </div>}
  </>
}
