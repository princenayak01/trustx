...
export async function registerUser(name: string, email: string, password: string) {
  ...
  const [existing] = await db.execute(sql`select id from trustx_auth_users where email = ${normalizedEmail} limit 1`)
  if (existing) throw new Error('An account with this email already exists.')

  const result = await db.execute(sql`insert into trustx_auth_users (full_name, email, password_hash)
    values (${name.trim()}, ${normalizedEmail}, ${hashPassword(password)})
...
export async function createSession(user: AuthUser) {
  ...
  await db.execute(sql`insert into trustx_auth_sessions (user_id, token_hash, expires_at)
    values (${user.id}, ${hashToken(token)}, ${expires})`)
  const store = await cookies()
  store.set(SESSION_COOKIE, token, {
    httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax',
    path: '/', expires, maxAge: SESSION_DAYS * 86400,
  })
  return user
}
...
export async function getCurrentUser(): Promise<AuthUser | null> {
  ...
  const result = await db.execute(sql`select u.id, u.full_name, u.email, u.role
    from trustx_auth_sessions s join trustx_auth_users u on u.id = s.user_id
