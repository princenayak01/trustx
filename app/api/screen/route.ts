...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json({ success: false, error: { code: 'FILE_REQUIRED', message: 'A document file is required.' } }, { status: 400 })
    }
    ...
    await db.insert(ocrResults).values({
      ...
      ocrEngine:ai?.ocr?.engine || 'trustx-ocr-adapter',
      confidence:ocrConfidence,
      extractedFields:{ documentType, mode:analysisMode, fields:ai?.ocr?.fields || [] },
    })

    stage = 'saving forensic result'
    await db.insert(forensicResults).values({
      screeningId:screening.id,
      tamperingProbability:Number(forensic.tampering_probability),
      compressionAnomaly:Number(forensic.compression_anomaly),
      copyMoveProbability:Number(forensic.copy_move_probability),
    ...
    if (analysis.factors.length) {
      await db.insert(riskFactors).values(analysis.factors.map((f) => ({
        screeningId:screening.id,
        factorType:f.type,
        severity:f.severity,
        score:f.score,
    ...
    const [updated] = await db.update(screenings).set({
      riskScore:analysis.score,
      riskLevel:analysis.level,
      status:finalStatus,
      modelVersion:analysis.modelVersion,
      processingCompletedAt:new Date(),
    }).where(eq(screenings.id, screening.id)).returning()
    ...
    await db.insert(auditLogs).values({
      ...
      metadata:{ documentHash:hash, mode:analysisMode },
    })
    ...
    const code = err.code || cause.code || 'UNKNOWN'
    const constraint = err.constraint || cause.constraint
    const detail = err.detail || cause.detail
    const rawMessage = err.message || 'Unable to process document.'
    const isAuth = rawMessage === 'Authentication required.'

    console.error('TrustX screening error', {
      stage,
      code,
      message: rawMessage,
      detail,
    ...
    if (isAuth) {
      return NextResponse.json({ success:false, error:{ code:'UNAUTHORIZED', message:rawMessage } }, { status:401 })
    }

    // Deliberately expose only diagnostic database metadata, never SQL text,
    // connection strings, query parameters, or uploaded document contents.
    const diagnostic = [
      `Upload failed during ${stage}.`,
      rawMessage,
      code !== 'UNKNOWN' ? `Database code: ${code}.` : '',
    ...
    return NextResponse.json({
      success:false,
      error:{ code:'SCREENING_FAILED', message:diagnostic },
    }, { status:400 })
  }
}
