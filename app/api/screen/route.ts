...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    stage='saving forensic result'; await db.insert(forensicResults).values({screeningId:screening.id,tamperingProbability:Number(forensic.tampering_probability),compressionAnomaly:Number(forensic.compression_anomaly),copyMoveProbability:Number(forensic.copy_move_probability),noiseInconsistency:Number(forensic.noise_inconsistency),metadataAnomaly:Number(forensic.metadata_anomaly)})
    stage='saving validation result'; await db.insert(validationResults).values({screeningId:screening.id,formatScore:Number(validation.format_score),structureScore:Number(validation.structure_score),fieldConsistencyScore:Number(validation.field_consistency_score),qrConsistencyScore:Number(validation.qr_consistency_score),dateConsistencyScore:Number(validation.date_consistency_score)})
    stage='saving risk factors'; if (analysis.factors.length) await db.insert(riskFactors).values(analysis.factors.map(f=>({screeningId:screening.id,factorType:f.type,severity:f.severity,score:f.score,description:f.description})))
    stage='updating screening result'; const finalStatus=analysis.level==='HIGH'||analysis.level==='CRITICAL'?'MANUAL_REVIEW':'COMPLETED'; const [updated]=await db.update(screenings).set({riskScore:analysis.score,riskLevel:analysis.level,status:finalStatus,modelVersion:analysis.modelVersion,processingCompletedAt:new Date()}).where(eq(screenings.id,screening.id)).returning()
    stage='writing audit log'; await db.insert(auditLogs).values({userId:ownerId,action:'SCREENING_CREATED',resourceType:'SCREENING',resourceId:screening.id,metadata:{documentHash:hash,mode:analysisMode}})
