...
import { documents, screenings, ocrResults, forensicResults, validationResults, riskFactors } from '@/lib/db/schema'

export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params
    const [screening] = await db.select({ screening: screenings, document: documents })
      .from(screenings)
    ...
    return NextResponse.json({ success: true, data: { ...screening, ocr: ocr[0] ?? null, forensic: forensic[0] ?? null, validation: validation[0] ?? null, factors }, message: 'Screening detail loaded' })
  } catch {
    return NextResponse.json({ success: false, error: { code: 'DATABASE_UNAVAILABLE', message: 'Screening detail is temporarily unavailable.' } }, { status: 503 })
  }
}
